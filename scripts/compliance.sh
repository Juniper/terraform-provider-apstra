...
if [ ! -x "$BIN" ]; then
  TMPDIR=$(mktemp -d)
  (cd tools/go-licenses && GOBIN="$TMPDIR" go install tool github.com/google/go-licenses/v2)
  mv "$TMPDIR/go-licenses" $BIN
  rm -rf "$TMPDIR"
fi
...
if [[ "$minimal_mode" == true ]]; then
    ...
    find "$TPC" -type f ! -name 'LICENSE*' ! -name 'NOTICE*' ! -name 'all_licenses.txt' -print0 | xargs -0 rm --
