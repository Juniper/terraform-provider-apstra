...
relative_script_path() {
    repo_root="$(git rev-parse --show-toplevel)" || fail "Failed to determine git repository root"
    script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
    script_name="$(basename -- "${BASH_SOURCE[0]}")"
    echo "${script_dir#"$repo_root"/}/$script_name"
}
...
gh pr create \
    ...
    --body "Automated release PR for release ${next_version} generated by $(relative_script_path)." \
    --label release
